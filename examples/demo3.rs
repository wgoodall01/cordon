//! # Demo: Chroot
//!
//! This demo runs the child in a mount namespace, and pivots root into a small chroot.
//!
//! - We're running `busybox sh` from the chroot, not `sh` from the host.
//! - Running `ls / /home` will show the filesystem in the chroot.
//! - Running `id` will show `root`.
//! - Running `cat /proc/$$/cgroup` will fail, because we don't have `/proc`
//! - Touching a file will create it in the chroot.
